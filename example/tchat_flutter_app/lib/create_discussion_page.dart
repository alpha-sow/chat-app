...
class _CreateDiscussionPageState extends State<CreateDiscussionPage> {
  ...
  String _generateDefaultTitle(Set<User> users) {
    ...
    final otherUsers = users.where((user) => user.id != widget.currentUser.id).toSet();
  ...
  void _createDiscussion() {
    ...
    if (_titleController.text.trim().isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Please enter a discussion title or select participants')),
      );
      return;
    }
    ...
    if (_selectedUsers.length <= 1) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Please select at least one other participant')),
      );
      return;
    }

    logger.i('Creating discussion: ${_titleController.text.trim()} (${_isCustomTitle ? "custom" : "auto-generated"})');
                ...
                itemBuilder: (context, index) {
                  ...
                  return Card(
                    child: CheckboxListTile(
                      value: isSelected,
                      onChanged: isCurrentUser ? null : (selected) {
                        setState(() {
                          if (selected == true) {
                      ...
                      secondary: CircleAvatar(
                        backgroundColor: isCurrentUser ? Colors.green[100] : Colors.blue[100],
                        child: Text(
                          user.initials,
                          style: TextStyle(
                            color: isCurrentUser ? Colors.green[800] : Colors.blue[800],
                            fontWeight: FontWeight.bold,
                          ),
                        ),
                      ),
                    ...
                    Wrap(
                      ...
                      children: _selectedUsers.map((user) {
                        ...
                        return Chip(
                          ...
                          backgroundColor: isCurrentUser ? Colors.green[50] : null,
                          side: isCurrentUser ? BorderSide(color: Colors.green[300]!) : null,
                        );
                      }).toList(),
                    ),
                  ],
